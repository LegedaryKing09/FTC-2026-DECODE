...
import org.firstinspires.ftc.teamcode.champion.controller.FastAutoTankDrive;
...
@Config
@Autonomous(name = "Blue Close Auton PRACTICE", group = "Competition")
public class StraightTest extends LinearOpMode {
    ...
    public static double INITIAL_BACKWARD = -45.0;
    ...
    public static boolean ENABLE_HEADING_CORRECTION = true;
    ...
    public void runOpMode() {
        ...
        tankDrive = new FastAutoTankDrive(hardwareMap, startPose);
    ...
    private void correctHeading(double targetHeadingDegrees) {
        if (!ENABLE_HEADING_CORRECTION || !opModeIsActive()) {
            return;
        }

        Pose2d currentPose = tankDrive.pinpointLocalizer.getPose();
        double currentHeadingRadians = currentPose.heading.toDouble();
        double currentHeadingDegrees = Math.toDegrees(currentHeadingRadians);

        // Normalize target heading to -180 to 180
        double normalizedTarget = normalizeAngle(targetHeadingDegrees);
        double normalizedCurrent = normalizeAngle(currentHeadingDegrees);

        // Calculate the shortest angle difference
        double headingError = normalizeAngle(normalizedTarget - normalizedCurrent);

        // Only correct if error is outside tolerance
        if (Math.abs(headingError) > HEADING_TOLERANCE && opModeIsActive()) {
            // Use RoadRunner's turn action for the correction
            Action correctionTurn = tankDrive.actionBuilder(currentPose)
                    .turn(Math.toRadians(headingError))
                    .build();
            Actions.runBlocking(correctionTurn);
    ...
    private void correctToCurrentPoseHeading() {
        if (!ENABLE_HEADING_CORRECTION || !opModeIsActive()) return;

        Pose2d pose = tankDrive.pinpointLocalizer.getPose();
        double headingDeg = Math.toDegrees(pose.heading.toDouble());

        correctHeading(headingDeg);
    }
